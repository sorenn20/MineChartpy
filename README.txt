## Visualizador y Analizador de Bloques Mineros

Este proyecto es una herramienta open-source en Python desarrollada con apoyo de inteligencia artificial, pensada para que cualquier persona —con o sin experiencia previa— pueda explorar, de forma sencilla, algunos conceptos fundamentales de la planificación minera a nivel de bloques. Aunque se trata de un programa básico y aún en desarrollo, permite visualizar y analizar información de forma amigable, acercando al usuario a procesos típicos del análisis minero.

### Funcionalidades principales
- Visualización 3D de bloques, a partir de archivos Excel con coordenadas, tamaño y leyes
- Filtrado de bloques según variables específicas definidas por el usuario
- Generación de curvas Tonelaje–Ley, utilizando densidad promedio y tamaño de bloque
- Reporte estadístico rápido de la variable seleccionada (media, varianza, histograma, etc.)

### Input esperado
Un archivo .xlsx con, al menos, las siguientes columnas:
- Coordenadas (X, Y, Z)
- Tamaño de bloque
- Variable de ley o atributo a analizar

### Estado del proyecto
En evolución. Se irán incorporando mejoras, mayor robustez y nuevas funciones conforme avance el desarrollo.

### Comentarios y contribuciones
¿Encontraste un error? ¿Tienes ideas para mejorar la herramienta?  
Toda sugerencia, issue o pull request es muy bienvenida.  
¡Gracias por pasar a revisar este proyecto!